...
const {document, EventTarget, HTMLScriptElement, Node} = global.window;
    ...
    class Runtime {
        ...
        */
        assignToNamespace(elt, nameSpace, name, value) {
            ...
            // if name has dash, camel case it
            // this is needed to easily access dasherized npm dependencies easily
            if (name.includes('-')) {
                name = name.replace(/-([a-z])/g, (stringParts) => stringParts[1].toUpperCase());
            }
            root[name] = value;
        }
